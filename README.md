...
>
> 0.9.0 在资源监控页加入 BE10000 SSH 状态与开关，关闭后会阻止持久化守护重新拉起 Dropbear，并同步关闭固件 SSH 开关。
...
- 在资源监控页查看 BE10000 SSH 的实际 LAN IPv4 监听、固件开关和持久化状态，并从已认证的控制台开启或关闭；关闭后不会被每分钟运行的 SSH 守护重新开启。
- SSH 开关所需的守护脚本标记与回滚顺序见 [BE10000 SSH 开关说明](docs/ssh-control.md)。
