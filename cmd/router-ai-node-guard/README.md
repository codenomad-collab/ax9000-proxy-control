...
v1.1.2 是运行在 ARM64 路由器本机的轻量守护程序，不依赖 PC 或 Codex 客户端持续在线。同一份静态二进制可用于 BE10000 与 AX9000；程序不判断机型。二进制位于 `<ShellClash>/tools/` 时会从自身位置发现根目录，也可用 `--shellcrash-root`、独立参数或 `NODE_GUARD_*` 环境变量覆盖。

正常情况下由 cron 每 30 分钟执行一次，检查：

- `AI-US-STABLE` 的结构、顺序和可用状态；GitHub 域名也由这个分组承载；
- 三个 AI 专用代理提供器是否仍各自匹配一个预期节点；
- 完整订阅中三个预期节点是否仍存在；
- 当前主线路对 OpenAI、Anthropic 和 GitHub 的可达性。

第一次运行不会读取 v1.0.0 的旧状态做切换。它会把旧 schema 视为未初始化，对当前订阅内全部符合协议和美国地区条件的候选执行一轮三目标探测，建立 v2 基线后才进入常规调度。后续普通故障先刷新提供器并记录；连续两次故障、托管分组不可用，或节点改名时才进行深度候选测试。
...
v1.1.2 的规则检查要求 `AI-US-STABLE` 至少保留合并后的 20 条基线，并逐条验证原有 8 个 GitHub 域名均走该组，且不再有规则指向 `GITHUB-US`；用户新增的精确分流规则不会被误判为自动修复失败。
