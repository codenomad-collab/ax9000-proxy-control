...
v1.1.1 是运行在 ARM64 路由器本机的轻量守护程序，不依赖 PC 或 Codex 客户端持续在线。同一份静态二进制可用于 BE10000 与 AX9000；程序不判断机型。二进制位于 `<ShellClash>/tools/` 时会从自身位置发现根目录，也可用 `--shellcrash-root`、独立参数或 `NODE_GUARD_*` 环境变量覆盖。
...
v1.1.1 的规则检查保留 AI 组 12 条、GitHub 组 8 条的最低基线；用户新增的精确分流规则不会被误判为自动修复失败。
